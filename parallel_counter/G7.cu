...
__global__ void G7_count_kernel(Node *graph, int numNodes, int totalComb,int l, int *count, int *combination) {
    ...
    if (tid < totalComb) {
        ...
        if((ij_connection && jk_connection && kl_connection && il_connection && jl_connection && !ik_connection)  ||
                    (ij_connection && jk_connection && kl_connection && il_connection && !jl_connection && ik_connection)  ||

                    (ik_connection && jk_connection && jl_connection && il_connection && kl_connection && !ij_connection)  ||
                    (ik_connection && jk_connection && jl_connection && il_connection && !kl_connection && ij_connection)  ||

                    (ij_connection && jl_connection && kl_connection && ik_connection && jk_connection && !il_connection) ||
                    (ij_connection && jl_connection && kl_connection && ik_connection && !jk_connection && il_connection)  ) {
...
int main() {
    ...
    cudaStatus = cudaMallocManaged(&comb, 128  * 3 * sizeof(int));
    //int num = 0;
    ...
    infile.read(reinterpret_cast<char*>(comb), 128  * 3 * sizeof(int));
    infile.close();
    ...
    int l;
    for(l = 0; l < 1138; l++){
        G7_count_kernel<<<gridSize, blockSize >>>(d_graph, numNodes, totalComb, l ,d_count, comb);
    }
    ...
    // Print the result
    printf("[main] Count: %d\n", h_count); // Counted 3 times because of implementation
