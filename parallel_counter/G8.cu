...
__global__ void G8_count_kernel(Node *graph, int numNodes, int totalComb,int l, int *count, int *combination) {
    ...
    if (tid < totalComb) {
        ...
        if( (ij_connection && ik_connection && il_connection && jk_connection && kl_connection && jl_connection) ) {
...
int main() {
    ...
    int l;
    for(l = 0; l < 1138; l++){
        G8_count_kernel<<<gridSize, blockSize >>>(d_graph, numNodes, totalComb, l ,d_count, comb);
    }
